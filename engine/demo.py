...
def demo_data():
    ...
    return {symbol: group.set_index("date").drop(columns="symbol")
            for symbol, group in frame.groupby("symbol", sort=False)}


def demo_backtest():
    return Backtest(["A", "B"], price_data=demo_data(), initial_capital=100,
                    strategy_cls=DemoStrategy, commission_per_share=0,
                    minimum_commission=1, slippage_bps=10)


def render_demo(result):
    lines = ["SYNTHETIC OHLC: 2 symbols x 4 daily bars; initial cash 100.00",
    ...
    for row in result["cash"].itertuples():
        ...
        for fill in fills[fills.fill_dt == row.dt].itertuples():
            lines.append(f"{day} OPEN {fill.direction} {fill.quantity} {fill.symbol} at "
                         f"{fill.fill_price:.3f}: cash flow {fill.cash_flow:.6f}")
        lines.append(f"{day} CLOSE cash={row.cash:.6f} reserved={row.reserved_cash:.6f} "
                     f"equity={row.equity:.6f}")
    return "\n".join(lines) + "\n"
